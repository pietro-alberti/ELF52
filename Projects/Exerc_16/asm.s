...
main    MOV R2, #PORTN_BIT                       ; Atribui R2 porta N
	LDR R0, =SYSCTL_RCGCGPIO_R               ; ENdereço de memoria do SYSCTL_RCGCGPIO_R
	LDR R1, [R0]                             ; leitura do estado anterior
	ORR R1, R2                               ; Habilita porta N
	STR R1, [R0]                             ; Atribuição do novo estado
        LDR R0, =SYSCTL_PRGPIO_R                 ; ENdereço de memoria do SYSCTL_RCGCGPIO_R
Aguarda	LDR R2, [R0]                             ; leitura do estado atual
	...
	LDR R0, =GPIO_PORTN_DIR_R                 ; ENdereço de memoria do GPIO_PORTN_DIR_R
	LDR R1, [R0]                              ; leitura do estado anterior
	ORR R1, R2                                ; OU de R1 e R2 (bit de saída)
	STR R1, [R0]                              ;  Atribuição do novo estado

	LDR R0, =GPIO_PORTN_DEN_R                 ; ENdereço de memoria do GPIO_PORTN_DEN_R
	LDR R1, [R0]                              ; leitura do estado anterior
	ORR R1, R2                                ; Habilita digital
	STR R1, [R0]                              ; Atribuição do novo estado

        MOV R1, #000000001b                       ; Atribui a R1 o estado inicial
 	LDR R0, = GPIO_PORTN_DATA_R               ; ENdereço de memoria do GPIO_PORTN_DATA_R
        MOV R2, #0x3FC

loop	
        LDR R3, [R0]                              ; Estado inciapl da porta N
        EOR R3, R1                                ; Altera o bit da saida do led
        STR R3, [R0]                              ; Salva a alteração a porta
